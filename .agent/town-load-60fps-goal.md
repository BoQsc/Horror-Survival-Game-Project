...
- The current fixed-seed validation run is already in a safe range: town peak is about `35.7 ms` with `0` frames over `40 ms` and `0` frames over `50 ms`, so the current load-time caching and spawn-resume path is doing the right kind of work.
...
- Prefer a small load-time cache or a tiny native helper over a clever scheduler if it gives the same result more simply.
...
- The fixed-seed route already validated that the spawn-resume split keeps the entry under control without starving visible objects.
...
- The latest fixed-seed run after the spawn-resume split landed at about `35.725 ms` peak with `0` frames over `40 ms` and `0` frames over `50 ms`, so the visible entrance is currently in the safe range.
