...
if "%TOWN_STALL_STRICT_LAUNCH_GUARDS%"=="1" (
    ...
    if errorlevel 1 (
        echo Existing Godot/town-stall/Python process found. Close it before launching Baseline V1 or unset TOWN_STALL_STRICT_LAUNCH_GUARDS.
        exit /b 2
    )
)
...
python -B addons\tests\run_town_stall_raw_baseline.py --cases runtime_deepidle60 --repeats 1 --hold-seconds 60 --idle-seconds 15 --sample-interval 1 --allow-contaminated-idle --max-gpu-temp-c 84 --preflight-max-gpu-temp-c 0 --preflight-cooldown-timeout-seconds 0 --preflight-cooldown-poll-seconds 15
