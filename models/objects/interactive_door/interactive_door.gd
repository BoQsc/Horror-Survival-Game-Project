...
func _disable_glb_collisions(door_model: Node):
	...
	if _cached_auto_static_body_paths.is_empty():
		# The preload pass already caches the scene tree paths. If no extra
		# autogenerated static bodies were found there, avoid rescanning the
		# entire imported door hierarchy on every spawn.
		if _cached_door_scene_ready:
			return
		_cache_door_scene_paths_recursive(door_model, door_model)
