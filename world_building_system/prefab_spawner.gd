...
func get_telemetry_snapshot() -> Dictionary:
	return {
		...
		"world_map_baked_building_key_count": _world_map_baked_building_payloads_by_key.size(),
		"world_map_baked_building_trigger_chunk_count": _world_map_baked_building_keys_by_terrain_chunk.size(),
		"world_map_baked_building_payload_chunk_count": _world_map_baked_building_keys_by_terrain_chunk.size(),
...
func _apply_existing_world_map_baked_buildings() -> void:
	if not terrain_manager or not building_manager:
		return
	_ensure_world_map_baked_building_payloads()
	if _world_map_baked_buildings_by_chunk.is_empty() and _world_map_baked_building_payloads_by_key.is_empty():
		return
...
func _queue_world_map_baked_building_payload(building_key: String, spawn_pos: Vector3) -> bool:
	if building_key.is_empty():
		return false
...
func _process_pending_world_map_baked_building_payloads() -> void:
	if _pending_world_map_baked_building_payloads.is_empty():
		_last_world_map_baked_payload_apply_ms = 0.0
		_last_world_map_baked_payload_apply_count = 0
		_flush_world_map_baked_payload_visuals_if_ready()
		return
	if not building_manager:
		return
	...
	var applied := 0
	var processed := 0
	var max_per_frame := maxi(1, world_map_baked_payload_max_per_frame)
	while not _pending_world_map_baked_building_payloads.is_empty() and processed < max_per_frame:
		if processed > 0:
			var elapsed_ms := float(Time.get_ticks_usec() - start_time) / 1000.0
			if elapsed_ms >= world_map_baked_payload_budget_ms:
				break

		var job: Dictionary = _pending_world_map_baked_building_payloads.pop_back()
		var building_key := str(job.get("building_key", ""))
		_pending_world_map_baked_building_payload_keys.erase(building_key)
		processed += 1
		if _apply_world_map_baked_building_payload_for_key(building_key, false):
			applied += 1

	_last_world_map_baked_payload_apply_ms = float(Time.get_ticks_usec() - start_time) / 1000.0
	_last_world_map_baked_payload_apply_count = applied
	_flush_world_map_baked_payload_visuals_if_ready()
...
func _flush_world_map_baked_payload_visuals_if_ready() -> void:
	...
	_last_world_map_baked_payload_flush_ms = float(Time.get_ticks_usec() - start_time) / 1000.0

func _apply_world_map_baked_building_payload_for_key(building_key: String, flush_visual_batches_now: bool = true) -> bool:
	...
	var chunk_payload: Dictionary = payload.get("chunk_payload", {})
	var object_spawns: Array = payload.get("object_spawns", [])
	var visual_payload: Dictionary = payload.get("visual_payload", {})
	building_manager.apply_world_map_baked_building_payload(chunk_payload, object_spawns, flush_visual_batches_now, flush_visual_batches_now, visual_payload, building_key)
	_spawned_world_map_baked_building_keys[building_key] = true
	return true
...
func _apply_world_map_baked_buildings(terrain_coord: Vector3i) -> void:
	...
	var handled_baked_buildings := false
	for bldg_variant in chunk_buildings:
		if typeof(bldg_variant) != TYPE_DICTIONARY:
			continue
		var bldg: Dictionary = bldg_variant
	...
	if applied_baked_buildings == 0 and has_baked_candidates and not handled_baked_buildings:
		for bldg_variant in chunk_buildings:
			if typeof(bldg_variant) != TYPE_DICTIONARY:
				continue
			var bldg: Dictionary = bldg_variant
	...
	if applied_baked_buildings == 0 and not has_unbaked_candidates and chunk_buildings.is_empty():
...
func _build_world_map_baked_building_voxel_payload(rotated_blocks: Array, spawn_pos: Vector3) -> Dictionary:
	if rotated_blocks.is_empty():
		return {}

	var block_positions: Array = []
	block_positions.resize(rotated_blocks.size())
	...
	for block_index in range(rotated_blocks.size()):
		var block_variant: Variant = rotated_blocks[block_index]
		if typeof(block_variant) != TYPE_DICTIONARY:
			continue
		var block: Dictionary = block_variant
		var rotated_offset: Vector3i = block.get("offset", Vector3i.ZERO)
		var block_global_pos := spawn_pos + Vector3(rotated_offset)
		var gx := int(floor(block_global_pos.x))
		var gy := int(floor(block_global_pos.y))
		var gz := int(floor(block_global_pos.z))
		var block_pos := Vector3i(gx, gy, gz)
		block_positions[block_index] = block_pos
		min_x = mini(min_x, gx)
		min_y = mini(min_y, gy)
		min_z = mini(min_z, gz)
		max_x = maxi(max_x, gx)
	...
	var edge_size := maxi(maxi(max_x - min_x + 1, max_y - min_y + 1), max_z - min_z + 1)
	if edge_size <= 0:
		return {}

	var voxel_count := edge_size * edge_size * edge_size
	...
	for block_index in range(rotated_blocks.size()):
		...
		if typeof(block_variant) != TYPE_DICTIONARY:
			continue
		var block: Dictionary = block_variant
		var block_pos_variant: Variant = block_positions[block_index]
		if typeof(block_pos_variant) != TYPE_VECTOR3I:
			continue
		var block_pos: Vector3i = block_pos_variant
		var local_x := block_pos.x - min_x
		var local_y := block_pos.y - min_y
		var local_z := block_pos.z - min_z
		if local_x < 0 or local_y < 0 or local_z < 0 or local_x >= edge_size or local_y >= edge_size or local_z >= edge_size:
			continue
		var local_index := local_x + local_y * edge_size + local_z * edge_size * edge_size
		voxel_bytes.encode_u8(local_index, int(block.get("type", 0)))
		voxel_meta.encode_u8(local_index, int(block.get("meta", 0)))

	return {
		"voxel_bytes": voxel_bytes,
		"voxel_meta": voxel_meta,
		"voxel_origin": Vector3(float(min_x), float(min_y), float(min_z)),
		"voxel_size": edge_size,
		"min_coord": Vector3i(min_x, min_y, min_z),
		"max_coord": Vector3i(max_x, max_y, max_z)
	}

func _get_world_map_baked_building_trigger_coords(min_coord: Vector3i, max_coord: Vector3i, chunk_stride: int = 31) -> Array:
	var trigger_coords: Array = []
	if chunk_stride <= 0 or min_coord.x > max_coord.x or min_coord.z > max_coord.z:
		return trigger_coords

	var min_chunk_x := int(floor(float(min_coord.x) / float(chunk_stride)))
	var max_chunk_x := int(floor(float(max_coord.x) / float(chunk_stride)))
	var min_chunk_z := int(floor(float(min_coord.z) / float(chunk_stride)))
	var max_chunk_z := int(floor(float(max_coord.z) / float(chunk_stride)))
...
func _process_pending_spawn_jobs() -> void:
	...
	if building_manager and building_manager.has_method("has_dirty_global_visual_batches") and building_manager.has_dirty_global_visual_batches():
		...
		var has_pending_baked_object_spawns: bool = building_manager.has_method("has_pending_world_map_baked_object_spawns") and building_manager.has_pending_world_map_baked_object_spawns()
		if pending_spawn_jobs.is_empty() and _pending_world_map_baked_building_payloads.is_empty() and not has_pending_baked_object_spawns:
			building_manager.flush_global_visual_batches()
	...
	if building_manager.has_method("has_dirty_chunks") and building_manager.has_dirty_chunks():
		...
		var has_pending_baked_object_spawns_for_chunk_flush: bool = building_manager.has_method("has_pending_world_map_baked_object_spawns") and building_manager.has_pending_world_map_baked_object_spawns()
		if has_visible_dirty_chunks and pending_spawn_jobs.is_empty() and _pending_world_map_baked_building_payloads.is_empty() and not has_pending_baked_object_spawns_for_chunk_flush and (processed > 0 or _last_spawn_job_msec > 0) and idle_since_last_spawn >= int(chunk_flush_interval_ms):
			building_manager.flush_dirty_chunks()
	_last_spawn_processing_ms = float(Time.get_ticks_usec() - start_time) / 1000.0
	_last_spawn_jobs_processed = processed
...
func _sort_world_map_baked_payload_job_by_distance(a: Dictionary, b: Dictionary) -> bool:
	var a_dist := float(a.get("distance_sq", 0.0))
	var b_dist := float(b.get("distance_sq", 0.0))
	if a_dist == b_dist:
		return str(a.get("building_key", "")) < str(b.get("building_key", ""))
	return a_dist > b_dist

func _get_world_map_baked_building_distance_sq(spawn_pos: Vector3) -> float:
	...
	return origin.distance_squared_to(spawn_pos)
