...
void main() {
    ...
    if (params.shape_type == 2) {
        ...
        // DIAMOND Shape (Octahedron) - Manhattan distance, binary density
        // Creates crystalline, gem-like breaking pattern (like 7 Days to Die)
        vec3 dist_vec = abs(world_pos - params.brush_pos.xyz);
        float manhattan_dist = dist_vec.x + dist_vec.y + dist_vec.z;
        
        if (manhattan_dist <= params.brush_pos.w) {
            // Binary density for crisp, faceted edges (no gradual falloff)
            density_buffer.values[index] = params.brush_value;
            modified = true;
        }
    } else {
    ...
    if (params.material_id >= 0 && params.brush_value < 0.0) {
        ...
        if (params.shape_type == 3) {
            // DIAMOND: use Manhattan distance for material too (prevents spill)
            float manhattan = dist_vec.x + dist_vec.y + dist_vec.z;
            float material_radius = brush_radius + 0.49;
            should_write = (manhattan <= material_radius);
    ...
    // AGGRESSIVE QUANTIZATION: Snap density to discrete levels for blocky 7DTD-like feel
    // This creates sharp transitions instead of smooth gradients
    if (modified) {
        float current = density_buffer.values[index];
        
        // Quantize to discrete steps: forces hard edges
        // Use a quantization step size (0.5 = moderate, 1.0 = aggressive)
        float step_size = 0.5;
        current = round(current / step_size) * step_size;
        
        // Further snap near the surface (where iso-surface = 0)
        // This makes block boundaries extremely crisp
        if (abs(current) < 1.0) {
            current = sign(current) * step_size;  // Force to +/-0.5
        }
        
        density_buffer.values[index] = current;
    }
